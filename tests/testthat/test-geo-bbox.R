...
test_that("misssing values have the correct bounding boxes", {
  expect_identical(geo_bbox(NA_wkt_), NA_rect_)
  expect_identical(geo_bbox(NA_wkb_), NA_rect_)
  expect_identical(geo_bbox(NA_collection_), NA_rect_)
  # tricky corner case...POINT EMPTY is the same in GEOS as POINT (nan nan)
  # for safety, don't drop SRID
  expect_identical(geo_bbox(NA_xy_), geo_rect(NA, NA, NA, NA, srid = 0))
  expect_identical(geo_bbox(NA_segment_), NA_rect_)
  expect_identical(geo_bbox(NA_rect_), NA_rect_)
