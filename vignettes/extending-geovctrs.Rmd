...
is_linestring.default <- function(x) {
  if (is_geovctr(x)) {
    geo_geometry_type(as_wksxp(x)) == "linestring"
  } else {
...
is_linestring(geo_segment(0, 0, 1, 1))
...
You'll notice that "geovctr" here isn't a class (for a variety of reasons), but has a few things you can count on: `is_geovctr()` will always return `TRUE`, and it will always be coercible to `wk::wkt()`, `wk::wkb()`, and `wk::wksxp()`. In this example, we take advantage of the guarantee of a `as_wksxp()` method to implement a default method that will work for any "geovctr".
...
The geovctrs package re-exports three "generic" geometry vector classes from the [wk package](https://paleolimbot.github.io/wk) (`wkt()`, `wkb()`, and `geo_collection()` can represent almost any point, line, or polygon feature) and three simple geometry vector classes (`geo_xy()`, `geo_segment()`, and `geo_rect()` are all efficient representations of specific types of features). To make a new type of vector work with geovctrs functions, you can either implement `as_geovctr()` and (optionally) `restore_geovctr()`.
